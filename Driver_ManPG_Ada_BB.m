...
if isfield(option, 'method')
    if strcmp(option.method, 'Raydan') ~=1 && strcmp(option.method, 'HongChao') ~=1  %逻辑与，如果method既不是Raydan也不是HongChao
        option.method = 'Raydan';                                                %则默认是Raydan方法
    end
else
    option.method = 'Raydan';
end
...
while(err > tol && f1 > Ftol + 1e-7 && iter < maxiter)
    ...
    while 1
        ...
        [f2, x2] = fhandle(x2);
        %非单调线搜索
        if f2  <= Cval - 0.5/t* alpha * deriv  || nls >= 10 %
            %btiter = btiter + 1;
            totalbt = totalbt + 1;
            break;
        end
    ...
    %x2.main=Transporter(x1.main,x2.main);
    %DD=Transporter(x1.main,DD);
