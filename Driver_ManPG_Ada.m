...
function [ xopt, iter, comtime, fopt, nD, sparsity, avar, fs,s] = driver(x0, A, mu, L, tol, maxiter, Ftol)
    ...
    xinitial = x0;
    tic
    [XX,xopt, fs, Ds,s] = solver(fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, xinitial, L, tol, delta, gamma, maxiter, mu, Ftol);
    comtime = toc;
...
function [XX,xopt, fs, Ds,s] = solver(fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, x0, L, tol, delta, gamma, maxiter, mu, Ftol)
    err = inf;
    x1 = x0;
    x2 = x1; fs = []; Ds = [];
    [f1, x1] = fhandle(x1);
